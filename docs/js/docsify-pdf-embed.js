...
! function () {
	...
	var renderer_func = function (code, lang, base = null) {
		var pdf_renderer = function (code, lang, verify) {
			...
			} else if (lang && !lang.localeCompare('3dmodel', 'en', { sensitivity: 'base' })) {
				if (verify) {
					return true;
				} else {
					var divId = "markdown_code_3dmodel_container_" + unique_id_generator().toString();
					var container_list = new Array();
					if (localStorage.getItem('3dmodel_container_list')) {
						container_list = JSON.parse(localStorage.getItem('3dmodel_container_list'));
					}
					container_list.push({ "3dmodel_location": code, "div_id": divId });
					localStorage.setItem('3dmodel_container_list', JSON.stringify(container_list));
					return (
						'<div style="margin-top:' + PDF_MARGIN_TOP + '; margin-bottom:' + PDF_MARGIN_BOTTOM + ';" id="' + divId + '">'
						+ '<a href="' + code + '"> Download </a> to ' + code +
						'</div>'
					);
				}
	...
	window.$docsify.plugins = [
		function (hook, vm) {
			...
			hook.afterEach(function (html, next) {
				lang = localStorage.getItem('code-lang');
				if (lang == 'pdf') {
					...
					if (container_list) {
						...
						if (mobile_view) {
							...
							container_list.forEach(function (container) {
								...
								var view_location = "/tools/pdfjs/web/viewer.html?file=" + absolute_pdf_location;
				...
				} else if (lang == '3dmodel') {
					container_list = JSON.parse(localStorage.getItem('3dmodel_container_list'));
					if (container_list) {
						html += '<script>';
						container_list.forEach(function (container) {
							var absolute_3d_location = url_check(container['3dmodel_location']) ? container['3dmodel_location'] : build_absolute_path(container['3dmodel_location']);
							var rvs = Math.floor(Math.random() * 10000);
							rvs = Math.floor(Math.random() * 10000) + rvs;
							var filename = getFilenameFromUrl(absolute_3d_location);
							html += '\
									let parentDiv_'+ rvs + ' = document.getElementById("' + container['div_id'] + '"); \
									parentDiv_'+ rvs + '.classList.add("model3d-down"); \
									parentDiv_'+ rvs + '.innerHTML = " \
									<a href=\\"'+ absolute_3d_location.replace('//', '\\\/\\\/') + '\\">Download ' + filename + '</a> </br>\
									"; \
									parentDiv_'+ rvs + '.style.height = "40rem"; \
									OV.SetExternalLibLocation ("/tools/o3dv/libs"); \
									let viewer_'+ rvs + ' = new OV.EmbeddedViewer(parentDiv_' + rvs + ', { \
										camera: new OV.Camera( \
											new OV.Coord3D(20, 20, 300.0), \
										...
										edgeSettings: new OV.EdgeSettings(false, new OV.RGBColor(0, 0, 0), 20), \
									}); \
									viewer_'+ rvs + '.Resize(); \
									viewer_'+ rvs + '.LoadModelFromUrlList(["' + absolute_3d_location + '"]); \
								;';
						});
						html += '<\/script>';
					}
